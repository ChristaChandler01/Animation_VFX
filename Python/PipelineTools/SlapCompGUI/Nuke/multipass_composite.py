# Save this file as multipass_composite.py in your Nuke plugin path (.nuke folder). This script 
# is designed to be called from Maya's quick_composite.py script, which collects user input 
# and camera data, then passes it to this script for building the Nuke composite.
...
def multipass_composite(input_path, 
    ...
    # Git but no matter which one I try my OCIO settings still do not match the school's and 
    # causes display errors. So I'm adding Detect OCIO availabilty for at home testing purposes 
    # to just get this turned in today.
    ocio_available = 'OCIO' in os.environ and os.path.exists(os.environ['OCIO'])
